...
export class LAppModel extends CubismUserModel {
  ...
  public update(): void {
    ...
    motionUpdated! = this._rightArmMotionManager.updateMotion(this._model, deltaTimeSeconds);
    motionUpdated! = this._leftArmMotionManager.updateMotion(this._model, deltaTimeSeconds);
  ...
  public startHandMotion(targetManage: CubismMotionManager, group: string, no: number, priority: number): CubismMotionQueueEntryHandle {
    ...
    //ex) idle_0
    let name: string = csmString.getFormatedString("{0}_{1}", group, no);
    let motion: CubismMotion = this._motions.getValue(name) as CubismMotion;
    let autoDelete = false;
    if (motion == null) {
    ...
    if (this._debugMode) {
      LAppPal.printLog("[APP]start motion: [{0}_{1}]", group);
    }
